...
def read_RSimages(mode, rescale=False):
    ...
    if not os.path.exists(img_A_dir):
        os.makedirs(img_A_dir)
    img_B_dir = os.path.join(root, mode, 'im2')
    if not os.path.exists(img_B_dir):
        os.makedirs(img_B_dir)
    label_A_dir = os.path.join(root, mode, 'label1')
    if not os.path.exists(label_A_dir):
        os.makedirs(label_A_dir)
    label_B_dir = os.path.join(root, mode, 'label2')
    if not os.path.exists(label_B_dir):
        os.makedirs(label_B_dir)
    # To use rgb labels:
...
class Data_test(data.Dataset):
    def __init__(self,  test_dir, mode):
        self.imgs_A = []
        self.imgs_B = []
        self.labels_A = []
        self.labels_B = []
        ...
        labels_A_dir = os.path.join(test_dir, 'label1')
        labels_B_dir = os.path.join(test_dir, 'label2')
